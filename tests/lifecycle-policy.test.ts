...
import { DEFAULT_LIFECYCLE_POLICY, resolveLifecyclePipeline } from '../src/lifecycle/policy.ts'
...
policy:
  lifecycle:
    enabled: true
    roles:
      planning: { provider: claude-code-worker, model: claude-opus-5, reasoning_effort: high, permission_preset: read-only, max_turns: 1 }
...
Work the task.`, '/tmp/WORKFLOW.md', options)
    ...
    expect(workflow.lifecycle?.roles.planning).toMatchObject({ model: 'claude-opus-5', permission_preset: 'read-only' })
